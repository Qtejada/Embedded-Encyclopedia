...
The BJT bias current flows through the skin resistance. This current produces a massive DC offset from *V = IR*.

The BJT current noise swamps the ECG signal in this example. This requirement excludes BJT inputs for the stated ECG application.
...
The 3.16 &micro;V noise is a substantial fraction of a 10 &micro;V EEG signal. Other noise sources can then bury the signal.
...
In a high-speed photodiode circuit, operational-amplifier voltage noise is often the primary error source. It exceeds the feedback-resistor noise in this condition.

Ignore noise density for a simple LED blink circuit. It is critical for these applications:
...
These are the critical failure mechanisms for the medical applications in these notes.
...
* **Selection:** Usually use a low-bias JFET or CMOS input with low 1/f noise.
  * Chopper and auto-zero amplifiers are frequent choices.

#### Gating: Slew Rate

* **Signal:** The system must detect the R-wave to trigger an MRI or CT scan.
  * The R-wave is the sharpest and fastest part of the cardiac cycle.
* **Pacemaker spike:** A pacemaker spike is extremely fast, shorter than 2 ms, and has a high voltage.
...
* **Selection:** Often select low operational-amplifier voltage noise to decrease this multiplication effect.
...
* **Core problem:** Most operational amplifiers use the Widlar topology.
  * The input stage drives a second stage that contains a compensation capacitor, *C*.

* **Limitation:** The input stage has a fixed maximum current, *I<sub>E</sub>*.
  * This current must charge or discharge the compensation capacitor.

* **Result:** When all available current flows into the capacitor, the output cannot change faster.
  * This limit is the **slew rate**:
...
1. **Noise-floor mask:** A distortion plot often looks flat at low frequencies.
...
2. **Input-capacitance distortion:** This problem occurs with a high-impedance source and some JFET amplifiers.
   ...
   * **Correction:** Use an inverting configuration with constant common-mode voltage, or match the source impedances as closely as possible to an ideal match.

3. **Gain-of-101 test:** A manufacturer often uses a noise gain of 101 to make a very small error measurable.
...
### Crossover Distortion: The Dead Zone
...
### Output Impedance: The Invisible Resistor
...
* **Variation with output voltage:** *R<sub>o</sub>* is typically highest when the output is near ground.
...
* **Characteristics:**
  ...
  * An input range that can usually extend beyond the supply rails.
...
* **R2 (10 k&Omega;): Input-current limiter**
  ...
  * This current limit prevents the protection diodes from burning out.
...
* **C1 (10 nF) and R5 (100 k&Omega;): Frequency-compensation network**
  ...
  * Without C1, the amplifier loses high-frequency feedback and oscillates.
...
* **U1:** An instrumentation amplifier with a configurable gain of **1–x**, as specified in the source notes.
...
* **High-impedance source such as ECG or pH:** For first-pass selection, ignore voltage noise and minimize current noise.
  * Minimize *i<sub>n</sub>* and use a FET input.
  * Include *e<sub>n</sub>* in the final noise budget after the topology selection.
* **Low-impedance source such as a strain gauge or power circuit:** For first-pass selection, ignore current noise and minimize voltage noise.
  * Minimize *e<sub>n</sub>* and use a BJT input.
  * Include *i<sub>n</sub>* in the final noise budget after the topology selection.
