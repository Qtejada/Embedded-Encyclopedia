...
The capacitor voltage decreases while it supplies the load. For small ripple and approximately constant load current:

<LearningEquation tex={"\\Delta V\\approx\\frac{I_{load}}{2f_{line}C}"} />

**fline** is the AC frequency. Full-wave charging occurs twice during each AC cycle.
...
1. The secondary peak is approximately **12.73 V**.
