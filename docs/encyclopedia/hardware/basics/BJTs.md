...
## Things to double-check or clarify for yourself

These are conceptual/math points you might want to keep in mind:

1. **Feedback gain expression**

   - Exact formula: `Acl = A / (1 + A*B)`.  
   - Approximation: if `A*B >> 1`, then `Acl ≈ 1/B`.

2. **Current source wording**

   - In the current-source section, it’s really the **emitter resistor** `RE` that sets the current: `IC ≈ (VB - 0.7 V) / RE`.  
   - The collector resistor mainly drops voltage; it doesn’t set the current directly.

3. **Common-emitter gain**

   - `Av ≈ -RC / RE` is OK as a **rough** rule when `RE` is unbypassed and much larger than `re`.  
   - More precise: include `re` and any load: `Av ≈ -(RC || Rload) / (RE + re)`.

4. **“BJTs outperform FETs”**

   - True in a lot of small-signal analog situations, but not universally.  
   - It’s more accurate to say “BJTs often have higher gm and can offer very good noise and accuracy in many analog circuits.”

5. **Two-resistor high-gain explanation**

   - If you want this to be crystal clear, you might later rewrite that part with a specific example schematic and explicit resistor names/values, so the “re voltage divider” concept is easier to see.
